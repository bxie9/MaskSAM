...
class nnUNetPredictor(object):
    ...
    def _internal_get_sliding_window_slicers_sam2(self, image_size: Tuple[int, ...], patch_size: Tuple[int, ...]):
        slicers = []
    ...
    def _internal_maybe_mirror_and_predict_sam2(self, x: torch.Tensor, seg: torch.Tensor) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        prediction = self.network(x, seg)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network(torch.flip(x, axes), torch.flip(seg, axes)), axes)
            prediction /= (len(axes_combinations) + 1)
        return prediction
    ...
    def _internal_predict_sliding_window_return_logits_sam2(self,
                                                       data: torch.Tensor,
                                                       slicers,
                                                       do_on_device: bool = True,
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                workon_seg = seg[sl][None]
                workon_seg = workon_seg.to(self.device)

                prediction = self._internal_maybe_mirror_and_predict_sam2(workon, workon_seg)[0].to(results_device)
    ...
    def predict_sliding_window_return_logits_sam2(self, input_image: torch.Tensor, seg: torch.Tensor) \
            -> Union[np.ndarray, torch.Tensor]:
        with torch.no_grad():
            assert isinstance(input_image, torch.Tensor)
            ...
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                print("1, ", torch.unique(seg))
                seg, _ = pad_nd_image(seg, self.configuration_manager.patch_size,
                                                           'constant', {'value': 0}, True,
                                                           None)
                ...
                data, slicer_revert_padding = pad_nd_image(input_image, self.configuration_manager.patch_size,
                                                           'constant', {'value': 0}, True,
                                                           None)
                print("2, ", torch.unique(seg))

                set_patch_size = self.configuration_manager.patch_size # (32, *data.shape[2:])
                slicers = self._internal_get_sliding_window_slicers_sam2(data.shape[1:], set_patch_size)
                # slicers = self._internal_get_sliding_window_slicers(data.shape[1:])

                if self.perform_everything_on_device and self.device != 'cpu':
                    # we need to try except here because we can run OOM in which case we need to fall back to CPU as a results device
                    try:
                        predicted_logits = self._internal_predict_sliding_window_return_logits_sam2(data, slicers,
                                                                                               self.perform_everything_on_device, 
                                                                                               seg=seg, patch_size = set_patch_size)
                    except RuntimeError:
                        print(
                            'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits_sam2(data, slicers, False, 
                                                                                               seg=seg, patch_size = set_patch_size)
                else:
                    predicted_logits = self._internal_predict_sliding_window_return_logits_sam2(data, slicers,
                                                                                           self.perform_everything_on_device, 
